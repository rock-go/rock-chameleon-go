...
func (srv *Server) config(ctx Context) *gossh.ServerConfig {
	...
	if srv.ServerConfigCallback == nil {
		...
		cfg = srv.ServerConfigCallback(ctx)
	}
	...
	if srv.Version != "" {
		cfg.ServerVersion = "SSH-2.0-" + srv.Version
	}
	...
	if srv.KeyboardInteractiveHandler != nil {
		cfg.KeyboardInteractiveCallback = func(conn gossh.ConnMetadata, challenger gossh.KeyboardInteractiveChallenge) (*gossh.Permissions, error) {
			applyConnMetadata(ctx, conn)

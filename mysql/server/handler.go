...
import (
	...
	"github.com/rock-go/rock-chameleon-go/mysql/internal/sockstate"
...
func (h *Handler) doQuery(
	...
	if len(bindings) > 0 {
		...
		if err != nil {
			logger.Errorf("Error processing bindings for query %s: %s", query, err)
			return err
		}
	}
	...
	if err != nil {
		logger.Errorf("connection %d: error running query: %s", c.ConnectionID, err)
		return err
	}
...
rowLoop:
	for {
		...
		case err = <-errChan:
			...
			logger.Debugf("connection %d: got error %s", c.ConnectionID, err.Error())
			close(quit)
			return err
		case row := <-rowChan:
			...
			logger.Debugf("connection %d spooling result row %s", c.ConnectionID, outputRow)
			r.Rows = append(r.Rows, outputRow)
			r.RowsAffected++
		case <-timer.C:
			if h.readTimeout != 0 {
				// Cancel and return so Vitess can call the CloseConnection callback
				logger.Debugf("connection %d got timeout", c.ConnectionID)
				close(quit)
	...
	switch len(r.Rows) {
	case 0:
		logger.Debugf("connection %d returning empty result", c.ConnectionID)
	case 1:
		logger.Debugf("connection %d returning result %v", c.ConnectionID, r)
	}
...
func (h *Handler) pollForClosedConnection(c *mysql.Conn, errChan chan error, quit chan struct{}) {
	...
	if !ok {
		logger.Debug("Connection checker exiting, connection isn't TCP")
		return
	}
	...
	if !ok {
		logger.Debug("Connection checker exiting, could not get local port")
		return
	}
	...
	for {
		...
		case sockstate.Broken:
			logger.Trace("socket state is broken, returning error")
			errChan <- ErrConnectionWasClosed.New()
			return
		case sockstate.Error:
			logger.Infof("Connection checker exiting, got err checking sockstate: %v", err)
			return
		default: // Established
...
func (h *Handler) handleKill(conn *mysql.Conn, query string) (bool, error) {
	...
	logger.Debugf("killing query %s", query)
	...
	if s[1] != "query" {
		logger.Infof("kill connection: id %d", connID)
		h.sm.CloseConn(conn)
		conn.Close()
	}

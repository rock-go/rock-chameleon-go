...
func (c *CreateIndex) createIndex(
	...
	err := driver.Save(ctx, index, newLoggingPartitionKeyValueIter(ctx, iter))
	close(done)
	...
	if err != nil {
		...
		//logger.WithField("err", err).Error("unable to save the index")

		deleted, err := ctx.DeleteIndex(index.Database(), index.ID(), true)
		if err != nil {
			ctx.Error(0, "unable to delete index: %s", err)
			//logger.WithField("err", err).Error("unable to delete the index")
		} else {
			<-deleted
		}
	} else {
...
func (i *loggingPartitionKeyValueIter) Next() (sql.Partition, sql.IndexKeyValueIter, error) {
	...
	return p, newLoggingKeyValueIter(i.ctx, iter, &i.rows), nil
}
